...
class PrometheusCustomCollector():  # pylint: disable=too-few-public-methods
    ...
    def delta_compared_to_max(self, source_metric, target_metric_name):
        metric = self._metrics_loader.universal_gauge(target_metric_name, 'This metrics finds delta for a target metric.')
        highest = 0
        ...
        for sample in source_metric.samples:
            ...
            metric.add_metric(sample[1], delta)
        return metric
    ...
    def collect(self):
        ...
        block_height_delta_metric = self.delta_compared_to_max(block_height_metric, 'brpc_block_height_behind_latest')
